  ...
  <div class="order-list-item" v-if="item.order_id">
    ...
    <div class="actions">
      <div v-if="item.order_status === 10">
        <span v-if="item.pay_status === 10" @click="payrightnow(e)">立刻付款</span>
        <span v-else-if="item.delivery_status === 10" @click="cancel">申请取消</span>
...
<script>
import {getMyOrderPay, MyorderCancel} from '@/api/order'
export default {
  props: {
    item: {
      type: Object,
  ...
  methods:{
   async payrightnow (e) {
      const res = await getMyOrderPay(this.item.order_id,10)
      console.log(res);
      console.log(this.item.order_id);
    },
    async cancel () {
      const res = await MyorderCancel(this.item.order_id)
      this.$toast(res.message + '请刷新查看')
    }
  }
}
</script>
